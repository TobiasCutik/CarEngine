...
namespace CE
{
    SedanCar::SedanCar(chrono::ChSystem* simulationSystem, glm::vec3 position, glm::vec3 rotation, glm::vec3 scale, std::unique_ptr<CarInputHandler> inputHandler,
        ...
        : Car(std::move(inputHandler))
    {
        ...
        chronoCar = sedan::Sedan(simulationSystem);
    ...
    void SedanCar::setCollisionModel(CollisionModel collision)
    {
        switch (collision)
        {
    ...
    void SedanCar::setTire(Tire tire)
    {
        switch (tire)
        {
            case Tire::RIGID:
    ...
    void SedanCar::setInitialChronoCarTransform(glm::vec3 position, glm::vec3 rotation)
    {
        chronoCar.SetInitPosition(chrono::ChCoordsys<>(Math::openGlToChronoPosition(position),
                                                       Math::openGlToChronoRotation(rotation)));
    }

    void SedanCar::initialize()
    {
        chronoCar.Initialize();
    }

    void SedanCar::synchronizeSimulation(const CarInputs& inputs, Terrain* terrain, double simulationTime)
    {
    ...
    void SedanCar::advanceSimulation(float vehicleFixedDeltaTime)
    {
