...
void btAdjustInternalEdgeContacts(btManifoldPoint& cp, const btCollisionObjectWrapper* colObj0Wrap, const btCollisionObjectWrapper* colObj1Wrap, int partId0, int index0, int normalAdjustFlags)
{
	...
	{
		btVector3 nearest01;
		btNearestPointInLineSegment(cp.m_localPointB, v0, v1, nearest01);
		btScalar len = (contact - nearest01).length();
		//
	...
	{
		btVector3 nearest12;
		btNearestPointInLineSegment(cp.m_localPointB, v1, v2, nearest12);
		btScalar len = (contact - nearest12).length();
		//
		if (len < disttobestedge)
		{
			bestedge = 1;
	...
	{
		btVector3 nearest20;
		btNearestPointInLineSegment(cp.m_localPointB, v2, v0, nearest20);
		btScalar len = (contact - nearest20).length();
		//
		if (len < disttobestedge)
		{
			bestedge = 2;
		...
		if (len < triangleInfoMapPtr->m_edgeDistanceThreshold)
			if (bestedge == 1)
			{
				...
				else
				{
					...
					{
						numConvexEdgeHits++;
						btVector3 localContactNormalOnB12 = colObj0Wrap->getWorldTransform().getBasis().transpose() * cp.m_normalWorldOnB;
						btVector3 clampedLocalNormal;
						bool isClamped = btClampNormal(edge, swapFactor * tri_normal, localContactNormalOnB12, info->m_edgeV1V2Angle, clampedLocalNormal);
						if (isClamped)
						{
							if (((normalAdjustFlags & BT_TRIANGLE_CONVEX_DOUBLE_SIDED) != 0) || (clampedLocalNormal.dot(frontFacing * tri_normal) > 0))
							{
			...
			if (bestedge == 2)
			{
				...
				else
				{
					...
					else
					{
						...
						btVector3 localContactNormalOnB20 = colObj0Wrap->getWorldTransform().getBasis().transpose() * cp.m_normalWorldOnB;
						btVector3 clampedLocalNormal;
						bool isClamped = btClampNormal(edge, swapFactor * tri_normal, localContactNormalOnB20, info->m_edgeV2V0Angle, clampedLocalNormal);
						if (isClamped)
						{
							if (((normalAdjustFlags & BT_TRIANGLE_CONVEX_DOUBLE_SIDED) != 0) || (clampedLocalNormal.dot(frontFacing * tri_normal) > 0))
							{
