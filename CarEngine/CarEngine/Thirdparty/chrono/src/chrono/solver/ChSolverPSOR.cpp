...
double ChSolverPSOR::Solve(std::shared_ptr<ChSystemDescriptor> sysd) {
    ...
    for (unsigned int ic = 0; ic < sysd->vconstraints.size(); ic++)
        sysd->vconstraints[ic]->Update_auxiliary();
    ...
    double gi_values[3];
    for (unsigned int ic = 0; ic < sysd->vconstraints.size(); ic++) {
        if (sysd->vconstraints[ic]->GetMode() == CONSTRAINT_FRIC) {
            gi_values[j_friction_comp] = sysd->vconstraints[ic]->Get_g_i();
            j_friction_comp++;
            if (j_friction_comp == 3) {
                double average_g_i = (gi_values[0] + gi_values[1] + gi_values[2]) / 3.0;
                sysd->vconstraints[ic - 2]->Set_g_i(average_g_i);
                sysd->vconstraints[ic - 1]->Set_g_i(average_g_i);
                sysd->vconstraints[ic - 0]->Set_g_i(average_g_i);
                j_friction_comp = 0;
            }
        }
    }
    ...
    for (unsigned int iv = 0; iv < sysd->vvariables.size(); iv++) {
        if (sysd->vvariables[iv]->IsActive())
            sysd->vvariables[iv]->Compute_invMb_v(sysd->vvariables[iv]->Get_qb(), sysd->vvariables[iv]->Get_fb());  // q = [M]'*fb
    }
    ...
    if (m_warm_start) {
        for (unsigned int ic = 0; ic < sysd->vconstraints.size(); ic++)
            if (sysd->vconstraints[ic]->IsActive())
                sysd->vconstraints[ic]->Increment_q(sysd->vconstraints[ic]->Get_l_i());
    } else {
        for (unsigned int ic = 0; ic < sysd->vconstraints.size(); ic++)
            sysd->vconstraints[ic]->Set_l_i(0.);
    }
    ...
    for (int iter = 0; iter < m_max_iterations; iter++) {
        ...
        for (unsigned int ic = 0; ic < sysd->vconstraints.size(); ic++) {
            ...
            if (sysd->vconstraints[ic]->IsActive()) {
                ...
                double mresidual = sysd->vconstraints[ic]->Compute_Cq_q() + sysd->vconstraints[ic]->Get_b_i() +
                                   sysd->vconstraints[ic]->Get_cfm_i() * sysd->vconstraints[ic]->Get_l_i();

                // true constraint violation may be different from 'mresidual' (ex:clamped if unilateral)
                double candidate_violation = fabs(sysd->vconstraints[ic]->Violation(mresidual));

                // compute:  delta_lambda = -(omega/g_i) * ([Cq_i]*q + b_i + cfm_i*l_i )
                double deltal = (m_omega / sysd->vconstraints[ic]->Get_g_i()) * (-mresidual);

                if (sysd->vconstraints[ic]->GetMode() == CONSTRAINT_FRIC) {
                    candidate_violation = 0;

                    // update:   lambda += delta_lambda;
                    old_lambda_friction[i_friction_comp] = sysd->vconstraints[ic]->Get_l_i();
                    sysd->vconstraints[ic]->Set_l_i(old_lambda_friction[i_friction_comp] + deltal);
                    i_friction_comp++;
                    ...
                    if (i_friction_comp == 3) {
                        sysd->vconstraints[ic - 2]->Project();  // the N normal component will take care of N,U,V
                        double new_lambda_0 = sysd->vconstraints[ic - 2]->Get_l_i();
                        double new_lambda_1 = sysd->vconstraints[ic - 1]->Get_l_i();
                        double new_lambda_2 = sysd->vconstraints[ic - 0]->Get_l_i();
                        ...
                        if (m_shlambda != 1.0) {
                            ...
                            sysd->vconstraints[ic - 2]->Set_l_i(new_lambda_0);
                            sysd->vconstraints[ic - 1]->Set_l_i(new_lambda_1);
                            sysd->vconstraints[ic - 0]->Set_l_i(new_lambda_2);
                        }
                        ...
                        sysd->vconstraints[ic - 2]->Increment_q(true_delta_0);
                        sysd->vconstraints[ic - 1]->Increment_q(true_delta_1);
                        sysd->vconstraints[ic - 0]->Increment_q(true_delta_2);
                    ...
                    // update:   lambda += delta_lambda;
                    double old_lambda = sysd->vconstraints[ic]->Get_l_i();
                    sysd->vconstraints[ic]->Set_l_i(old_lambda + deltal);
                    ...
                    sysd->vconstraints[ic]->Project();

                    // After projection, the lambda may have changed a bit..
                    double new_lambda = sysd->vconstraints[ic]->Get_l_i();

                    // Apply the smoothing: lambda= sharpness*lambda_new_projected + (1-sharpness)*lambda_old
                    if (m_shlambda != 1.0) {
                        new_lambda = m_shlambda * new_lambda + (1.0 - m_shlambda) * old_lambda;
                        sysd->vconstraints[ic]->Set_l_i(new_lambda);
                    }
                    ...
                    sysd->vconstraints[ic]->Increment_q(true_delta);
