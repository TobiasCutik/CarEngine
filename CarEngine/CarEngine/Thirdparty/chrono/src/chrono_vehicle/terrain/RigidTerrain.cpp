...
std::shared_ptr<RigidTerrain::Patch> RigidTerrain::AddPatch(int id,
                                                            std::shared_ptr<ChMaterialSurface> material,
...
// -----------------------------------------------------------------------------
// Removal of terrain patches
// -----------------------------------------------------------------------------

void RigidTerrain::RemovePatch(const std::shared_ptr<Patch>& patch) {
    auto itr = std::find(std::begin(m_patches), std::end(m_patches), patch);
    assert(itr != m_patches.end());
...
void RigidTerrain::Synchronize(double time) {
    ResetPatchesCollidingWithTires();
}
...
bool RigidTerrain::FindPoint(const ChVector<> loc, double& height, ChVector<>& normal, float& friction) const {
    ...
    for (auto patch : m_patches) {
        ...
        if (phit && pheight > height) {
            patch->m_collidingWithTire = true;
            hit = true;
            height = pheight;
            normal = pnormal;
...
std::vector<int> RigidTerrain::GetPatchesCollidingWithTires()
{
    std::vector<int> patches;

    for (const auto& patch : m_patches) {
        if (patch->m_collidingWithTire)
            patches.emplace_back(patch->m_id);
    }

    return patches;
}

void RigidTerrain::ResetPatchesCollidingWithTires() {
    for (const auto& patch : m_patches) {
        patch->m_collidingWithTire = false;
    }
}
