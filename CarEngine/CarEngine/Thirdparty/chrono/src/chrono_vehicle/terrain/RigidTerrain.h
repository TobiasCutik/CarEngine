...
class CH_VEHICLE_API RigidTerrain : public ChTerrain {
  public:
    ...
    class CH_VEHICLE_API Patch {
      ...
      protected:
        ...
        int m_id;                        ///< unique patch id (set via CarEngine)
        bool m_collidingWithTire;        ///< indicates if this patch collided with a tire in the last simulation update
                                         ///< WARNING: when all tires are deleted at runtime, this property may have an incorrect value, as tires (indirectly) update this property
    ...
    /// Resets m_collidingWithTire property of Patches
    ...
    /// Remove a terrain patch from this rigid terrain.
    void RemovePatch(const std::shared_ptr<Patch>& patch);
    ...
    std::vector<int> GetPatchesCollidingWithTires();
  ...
  private:
    ...
    // Resets m_collidingWithTire property of Patches
    // It is called in Synchronize()
    void ResetPatchesCollidingWithTires();
