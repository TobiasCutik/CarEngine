...
void ChWheeledVehicle::Synchronize(double time, const ChDriver::Inputs& driver_inputs, const ChTerrain& terrain) {
    ...
    // Modify the acceleration of the vehicle
    if (m_powertrain->GetDriveMode() == ChPowertrain::DriveMode::FORWARD)
        powertrain_torque = (powertrain_torque + m_acceleration_addend) * m_acceleration_multiplier;
