...
class CH_VEHICLE_API ChWheeledVehicle : public ChVehicle {
  public:
    ...
    /// Set vehicle acceleration addend. The value is added to the vehicle torque in each simulation step.
    void SetAccelerationAddend(float addend) { m_acceleration_addend = addend; }

    /// Set vehicle acceleration multiplier. The value is multiplied with the vehicle torque in each simulation step.
    void SetAccelerationMultiplier(float multiplier) { m_acceleration_multiplier = multiplier; }
  ...
  protected:
    ...
    float m_acceleration_addend = 0.f;           ///< is added to powertrain_torque in each simulation step (if driveMode is forward)
    float m_acceleration_multiplier = 1.f;       ///< is multiplied with powertrain_torque in each simulation step (if driveMode is forward)
};
